...
function goalToTask(g: { id: string; title: string; status: string; ownerName: string | null; dueDate: Date | null }) {
  const status = g.status === 'Completed' ? 'done' : g.status === 'InProgress' ? 'in-progress' : 'todo';
  return {
    id: g.id,
    title: g.title,
...
export async function GET(req: NextRequest) {
  ...
  const data = initiatives.map(ini => ({
    ...
    tasks: ini.goals.map(goalToTask),
