...
export function SocietyPostComments({ postId, userId, societyId, isMember }: SocietyPostCommentsProps) {
  ...
  const handleOverlayReply = async (parentId: string) => {
    if (!overlayReplyText.trim() || !session?.user?.id || !isMember) return;
  ...
  const handleSubmitComment = async () => {
    if (!newComment.trim() || !session?.user?.id || !isMember) return;
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/societies/${societyId}/posts/${postId}/comments`, {
        method: 'POST',
  ...
  const handleSubmitReply = async () => {
    if (!replyText.trim() || !session?.user?.id || !replyingTo || !isMember) return;
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/societies/${societyId}/posts/${postId}/comments`, {
        method: 'POST',
  ...
  return (
    <div className="mt-4 relative">
      ...
      {session?.user?.id && !isMember && (
        <div className="mb-4 text-center p-3 bg-muted/50 rounded-lg">
          <p className="text-sm text-muted-foreground">
            Join this society to participate in discussions and comment on posts.
          </p>
        </div>
      )}
      ...
      {showComments && (
        <div className="absolute inset-0 z-30 flex flex-col bg-card/90 backdrop-blur-sm text-card-foreground overflow-hidden">
          ...
          {/* Comment form - Sticks to bottom - only for members */}
          {isMember ? (
            ...
            <div className="p-3 border-t bg-card/95 flex-shrink-0 text-center">
              <p className="text-sm text-muted-foreground">
                Join this society to participate in discussions.
              </p>
            </div>
          )}
        </div>
