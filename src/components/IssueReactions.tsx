...
export default function IssueReactions({ 
  ...
  useEffect(() => {
    async function fetchCommentCount() {
      try {
        const commentRes = await fetch(`/api/issues/comments?issueId=${issueId}`);
  ...
  const handleChampion = async () => {
    ...
    try {
      const response = await fetch(`/api/issues/${issueId}/champion`, {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-row gap-2 lg:flex-col lg:gap-4">
        ...
        >
          <Heart size={20} className={cn("lg:w-6 lg:h-6", isChampioned ? "fill-current" : "")} />
