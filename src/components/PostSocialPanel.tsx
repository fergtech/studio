...
export default function PostSocialPanel({ postId, currentUserId, postType = 'general', societyId }: PostSocialPanelProps) {
    ...
    fetch(commentsApi)
      ...
      .then(data => {
        ...
        setComments(commentsArray.map((c: any) => ({
          ...
          text: c.text,
          timestamp: c.timestamp || c.createdAt // Handle both timestamp and createdAt
        })));
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    console.log('PostSocialPanel - Submitting comment:', { postId, userId: currentUserId, text: newComment, postType, societyId });
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log('PostSocialPanel - Comment created successfully:', comment);
        setNewComment('');
        ...
        fetch(commentsApi)
          .then(res => res.json())
          .then(data => {
            console.log('PostSocialPanel - Refetched comments:', data);
            const commentsArray = data.allComments || (Array.isArray(data) ? data : []);
            setComments(commentsArray.map((c: any) => ({
              id: c.id,
              userId: c.userId,
        ...
        console.error('PostSocialPanel - Failed to create comment:', res.status, errorData);
      }
    } catch (error) {
      console.error('PostSocialPanel - Error submitting comment:', error);
    }
  };
